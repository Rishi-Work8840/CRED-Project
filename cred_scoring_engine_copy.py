...
if __name__ == "__main__":
    ...
    # ─────────────────────────────────────────────────────────────
    # SECTION 10 — LLM COMPARISON METRICS
    # ─────────────────────────────────────────────────────────────
    ...
    df_compare = tech[["report_id", "cred_label"]].copy()
    df_compare["llm_label"] = df_compare["report_id"].map(llm_labels)
    df_compare["match"] = df_compare["cred_label"] == df_compare["llm_label"]

    total = len(df_compare)
    correct = df_compare["match"].sum()
    accuracy = correct / total * 100
    categories = ["Retain", "Consolidate", "Eliminate", "Develop"]

    print("\n")
    print("=" * 70)
    print("   CRED SCORING ENGINE vs LLM PREDICTIONS - COMPARISON METRICS")
    print("=" * 70)
    ...
    for cat in categories:
        ...
        fp = ((df_compare["cred_label"] == cat) & (df_compare["llm_label"] != cat)).sum()
        fn = ((df_compare["cred_label"] != cat) & (df_compare["llm_label"] == cat)).sum()

        precision = tp / (tp + fp) * 100 if (tp + fp) > 0 else 0
        recall = tp / (tp + fn) * 100 if (tp + fn) > 0 else 0
        f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0
        support = (df_compare["llm_label"] == cat).sum()
    ...
    for code_cat in categories:
        code_mask = df_compare["cred_label"] == code_cat
        print(f"   {code_cat:<14}", end="")
        row_total = 0
        for llm_cat in categories:
            llm_mask = df_compare["llm_label"] == llm_cat
    ...
    for llm_cat in categories:
        print(f" {(df_compare['llm_label'] == llm_cat).sum():>7}", end="")
    print(f"  {total:>6}")

    # ── 4. Misclassified Reports ──
    mismatches = df_compare[~df_compare["match"]].copy()
    print(f"\n4. MISCLASSIFIED REPORTS ({len(mismatches)} total)")
    print(f"   {'Report':<8} {'Code Says':<14} {'LLM Says':<14}")
    print(f"   {'-'*8} {'-'*14} {'-'*14}")
    for _, row in mismatches.iterrows():
        print(f"   {row['report_id']:<8} {row['cred_label']:<14} {row['llm_label']:<14}")

    # ── 5. Migration Impact Comparison ──
    print(f"\n5. MIGRATION IMPACT COMPARISON")
    code_elim = (df_compare["cred_label"] == "Eliminate").sum()
    code_cons = (df_compare["cred_label"] == "Consolidate").sum()
    llm_elim = (df_compare["llm_label"] == "Eliminate").sum()
    llm_cons = (df_compare["llm_label"] == "Consolidate").sum()
    ...
    for name, start, end in groups:
        ...
        group_df = df_compare[df_compare["report_id"].isin(group_ids)]
    ...
    with pd.ExcelWriter(metrics_file, engine="openpyxl") as writer:
        ...
        for code_cat in categories:
            ...
            for llm_cat in categories:
                count = ((df_compare["cred_label"] == code_cat) & (df_compare["llm_label"] == llm_cat)).sum()
                row_data[f"LLM: {llm_cat}"] = int(count)
        ...
        df_impact = pd.DataFrame([{
            ...
            "Consolidate": int(code_cons),
            "Migration Reduction (%)": round(code_saving, 1)
        }, {
            "Source": "LLM",
            "Eliminate": int(llm_elim),
            "Consolidate": int(llm_cons),
            "Migration Reduction (%)": round(llm_saving, 1)
        }])
        ...
        for name, start, end in groups:
            ...
            group_data.append({
                "Report Group": name,
                "Correct": int(group_df["match"].sum()),
                "Total": len(group_df),
                "Agreement (%)": round(group_df["match"].sum() / len(group_df) * 100, 1)
            })
